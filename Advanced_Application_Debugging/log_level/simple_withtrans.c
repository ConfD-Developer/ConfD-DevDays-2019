                     ...
                     int n)
{
  ...
  enum level {
    SILENT = smp_CONFD_SILENT,
    DEBUG = smp_CONFD_DEBUG,
    TRACE = smp_CONFD_TRACE,
    PROTO = smp_CONFD_PROTO_TRACE
  };
  
  enum level f;
  f = CONFD_GET_ENUM_VALUE(&params->v);
  
  switch(f) {
  case smp_CONFD_SILENT:
    confd_set_debug(CONFD_SILENT, stderr);
    break;
  case smp_CONFD_DEBUG:
    confd_set_debug(CONFD_DEBUG, stderr);
    break;
  case smp_CONFD_TRACE:
    confd_set_debug(CONFD_TRACE, stderr); 
    break;
  case smp_CONFD_PROTO_TRACE:
    confd_set_debug(CONFD_PROTO_TRACE, stderr); 
    break;
    pval(&params->v);
  default:
    return CONFD_ERR;
  }
  //confd_action_reply_values(uinfo, &reply, 16);
  return CONFD_OK;
}
...
int main(int argc, char *argv[]) {
    ...
    while ((oc = getopt(argc, argv, "sdtp")) != -1) {
        switch (oc) {
        case 's':
