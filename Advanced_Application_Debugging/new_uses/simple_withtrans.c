...
void mylogger(int syslogprio, const char *fmt, va_list ap) {
  ...
  strcat(buf, fmt);
  vfprintf(trace_fd, buf, ap);
}
*/
//static unsigned long lastprint = 0;

static void mylogger(int syslogprio, const char *fmt, va_list ap)
{
  char *buf;
  char mbuf[BUFSIZ];
  struct timeval now;
  ...
  /* Crude way to find out if there is a newline at the end of
   * the printout. */
  /*  {
    va_list ap1;
    char *fbuf;
                     ...
                     confd_value_t *keys, int nkeys)
{
  ...
  if (pos >= 0) {
    //fprintf(stderr, "POS> 0 = %d\n", pos);
    /* matching entry found - return its keys and 'pos' for next entry */
    CONFD_SET_UINT32(&v, (u_int32_t)running_db[pos].name);
...
int main(int argc, char *argv[]) {
    ...
    trace_fd = fopen("d.trace2", "a+");
    int debuglevel = CONFD_TRACE;

    int oc;             /* option character */

    while ((oc = getopt(argc, argv, "qdtps:")) != -1) {
        switch (oc) {
        case 'q':
